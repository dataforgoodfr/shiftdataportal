...
dst_filepath = os.path.join(GAPMINDER_DATA_PATH, "raw_population.csv")
df.to_csv(dst_filepath, index=False)

# Stage population data
df = gapminder.stage_population(GAPMINDER_DATA_PATH)
dst_filepath = os.path.join(GAPMINDER_DATA_PATH, "stg_population.csv")
df.to_csv(dst_filepath, index=False)
...
# 'population' folder.
df = gapminder.process_final_population(PROJECT_ROOT_PATH).reset_index(drop=True)
dst_filepath = os.path.join(GAPMINDER_DATA_PATH, "final_population.csv")
df.to_csv(dst_filepath, index=False)
