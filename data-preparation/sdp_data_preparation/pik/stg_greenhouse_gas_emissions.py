...
def stage_greenhouse_gas_emissions(data_filepath: str) -> pd.DataFrame:
    filepath = os.path.join(
        data_filepath, "raw_greenhouse_gas_emissions_third_party.csv"
    )
    ...
    df["greenhouse_gas_emission"] = (
        df["greenhouse_gas_emission"] * df["global_warming_potential"] / 1_000
    )
    df["unit"] = "MtCO2eq"
...
def _aggregate_and_format(df: pd.DataFrame) -> pd.DataFrame:
    df = (
        df.groupby(["country", "sector", "gas", "year", "unit"])
        .agg({"greenhouse_gas_emission": "sum"})
        .reset_index()
    )
