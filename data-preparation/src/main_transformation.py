...
DATA_DIR = os.path.join(os.path.dirname(os.path.dirname(CURRENT_DIR)), "data") # absolute equivalent of relative path "../../data"
RAW_DATA_DIR = os.path.join(DATA_DIR, "raw_data")  # contains the RAW data sources (have to be downloaded from Internet and placed in this folder)
RESULTS_DIR = os.path.join(DATA_DIR, "new_prod_data") # contains transformation results
CURRENT_DATA_DIR = os.path.join(DATA_DIR, "current_data")
CURRENT_PROD_DATA = os.path.join(DATA_DIR, "current_prod_data")  # contains TRANSFORMED old data generated by Dataiku.
...
class TransformationPipeline:
    ...
    def process_country_data(self):
        ...
        df_country = pd.read_csv(os.path.join(RAW_DATA_DIR, "country", "country_groups.csv"))
        df_country = df_country.sort_values(by=["group_type", "group_name", "country"])
        df_country.to_csv(os.path.join(RESULTS_DIR, "COUNTRY_country_groups_prod.csv"), index=False)
        return df_country
    ...
    def process_population_data(self, df_country):
        ...
        df_population.to_csv(os.path.join(RESULTS_DIR, "DEMOGRAPHIC_POPULATION_WORLDBANK_prod.csv"), index=False)

        # update GapMinder data (source GapMinder)
        df_population_gapmidner_raw = pd.read_excel(os.path.join(RAW_DATA_DIR, "population", "GM-Population - Dataset - v7.xlsx"),
        ...
        df_gapminder.to_csv(os.path.join(RESULTS_DIR, "DEMOGRAPHIC_POPULATION_GAPMINDER_prod.csv"), index=False)
    ...
    def process_ghg_data(self, df_country):
        ...
        # update EDGAR data
        df_edgar_gases = pd.read_excel(f"{CURRENT_PROD_DATA}/ghg/edgar_f_gases.xlsx")
        df_edgar_n2o = pd.read_excel(f"{CURRENT_PROD_DATA}/ghg/edgar_n2o_raw.xlsx")
        df_edgar_ch4 = pd.read_excel(f"{CURRENT_PROD_DATA}/ghg/edgar_ch4_raw.xlsx")
        df_edgar_co2_short_cycle = pd.read_excel(f"{CURRENT_PROD_DATA}/ghg/edgar_co2_shortcycle_raw.xlsx")
        df_edgar_co2_short_without_cycle = pd.read_excel(f"{CURRENT_PROD_DATA}/ghg/edgar_co2_withoutshortcycle_raw.xlsx")
        ...
        df_original_gas = pd.read_excel(f"{CURRENT_PROD_DATA}/ghg/ghg_full_by_gas_prod.xlsx")
        df_original_gas = StatisticsDataframeFormatter.select_and_sort_values(df_original_gas, "ghg", round_statistics=5)
        df_original_gas.to_csv(f"{CURRENT_PROD_DATA}/ghg/GHG_FULL_BY_GAS_prod.csv", index=False)

        df_original_sector = pd.read_excel(f"{CURRENT_PROD_DATA}/ghg/ghg_full_by_sector_prod.xlsx")
        ...
        df_original_agg = pd.read_excel(f"{CURRENT_PROD_DATA}/ghg/ghg_full_aggregated.xlsx")
