...
class GhgPikEdgarCombinator:
    ...
    def compute_pik_edgar_stacked(df_pik_clean, df_edgar_clean):
        ...
        df_pik_edgar_stacked = StatisticsDataframeFormatter().select_and_sort_values(df_pik_edgar_stacked, "ghg",
                                                                                     round_statistics=5)
    ...
    def compute_pik_edgar_filter_sector(self, df_pik_clean, df_edgar_clean):
        ...
        df_edgar_filter_sector = df_edgar_clean[
            df_edgar_clean["sector"].isin(["Transport", "Electricity & Heat", "Other Energy"])]
        ...
        df_pik_edgar_sector = StatisticsDataframeFormatter().select_and_sort_values(df_pik_edgar_sector, "ghg",
                                                                                    round_statistics=5)
    ...
    def merge_edgar_pik_per_sector(df_pik_clean, df_edgar_clean, pik_sectors, edgar_sectors):
        ...
        df_pik_edgar_merge_sector = df_pik_sector.merge(df_edgar_sector.drop(columns="ghg_unit"), how="inner",
                                                        on=["country", "year", "gas"], suffixes=("_pik", "_edgar"))
    ...
    def compute_pik_edgar_energy_ratio(self, df_pik_clean, df_edgar_clean):
        ...
        df_edgar_industry = df_edgar_clean[df_edgar_clean["sector"] == "Industry and Construction"]
        df_pik_industry = df_pik_clean[(df_pik_clean["sector"] == "Industry and Construction")]
        df_pik_edgar_diff_industry = df_edgar_industry.merge(df_pik_industry, on=["country", "year", "gas"],
                                                             suffixes=("_edgar", "_pik"))
        df_pik_edgar_diff_industry["ghg"] = df_pik_edgar_diff_industry["ghg_edgar"] - df_pik_edgar_diff_industry[
            "ghg_pik"]
        ...
        df_edgar_transport_energy = df_edgar_clean[
            df_edgar_clean["sector"].isin(["Transport", "Electricity & Heat", "Other Energy"])]
        df_pik_edgar_diff_industry_transport = pd.concat([df_pik_edgar_diff_industry, df_edgar_transport_energy],
                                                         axis=0)
        ...
        df_pik_edgar_agriculture = self.merge_edgar_pik_per_sector(df_pik_clean, df_edgar_clean, ["Agriculture"],
                                                                   ["Agriculture", "Other Agriculture"])
        df_pik_edgar_waste = self.merge_edgar_pik_per_sector(df_pik_clean, df_edgar_clean, ['Waste', 'Other Sectors'],
                                                             ['Waste', 'Other Sectors'])
        df_pik_edgar_industry = self.merge_edgar_pik_per_sector(df_pik_clean, df_edgar_clean,
                                                                ['Industry and Construction'], [
                                                                    'Industry and Construction'])  # TODO -vérifier si erreur dans la requete SQL car dans dans la requete SQL originale, on fait ici le ratio PIK par PIK

        # join and compute ratio EDGAT - PIK
        df_pik_edgar_ratio = pd.concat(
            [df_pik_energy_transport, df_pik_edgar_agriculture, df_pik_edgar_waste, df_pik_edgar_industry], axis=0)
        df_pik_edgar_ratio["ratio"] = df_pik_edgar_ratio["ghg_edgar"] / df_pik_edgar_ratio["ghg_pik"]

        return df_pik_edgar_ratio

    def compute_pik_edgar_extrapolated_glued(self, df_pik_clean,
                                             df_edgar_clean):  # TODO - revoir complètement cette méthode. Dette technique monstrueuse...
        ...
        df_pik_edgar_energy_ratio = df_pik_edgar_energy_ratio[
            (df_pik_edgar_energy_ratio["year"] >= "2008") & (df_pik_edgar_energy_ratio["year"] <= "2012")]
        df_pik_edgar_energy_ratio = df_pik_edgar_energy_ratio.groupby(
            ["country", "gas", "sector_edgar", "sector_pik"]).agg(averaged_ratio=("ratio", "mean")).reset_index()
        ...
        df_pik_edgar_energy_extrapolated["ghg_edgar_extrapolated"] = df_pik_edgar_energy_extrapolated[
                                                                         "averaged_ratio"] * \
                                                                     df_pik_edgar_energy_extrapolated["ghg_pik"]
        ...
        df_pik_edgar_energy_extrapolated = df_pik_edgar_energy_extrapolated.rename(
            columns={"sector_edgar": "sector", "ghg_edgar_extrapolated": "ghg"})
        list_col_extrapolation_to_concat = ["country", "sector", "gas", "ghg_unit", "year", "ghg"]
        df_pik_edgar_extrapolated_computed = pd.concat(
            [df_pik_edgar_energy_extrapolated[list_col_extrapolation_to_concat],
             df_pik_2012])
        ...
        df_pik_edgar_extrapolated = df_pik_edgar_extrapolated_computed[
            ["source", "country", "sector", "gas", "year", "ghg", "ghg_unit"]]
        df_pik_edgar_extrapolated["sector"] = np.where(df_pik_edgar_extrapolated["sector"] == "Energy from Industry",
                                                       "Industry and Construction", df_pik_edgar_extrapolated["sector"])
        ...
        df_pik_edgar_extrapolated_glued = StatisticsDataframeFormatter().select_and_sort_values(
            df_pik_edgar_extrapolated_glued, "ghg", round_statistics=5)
...
class EdgarUnfcccAnnexesCombinator:
    ...
    def run(self, df_edgar_clean, df_unfccc_annex_clean, df_country):
        ...
        df_edgar_unfccc_stacked_per_zones_and_countries = StatisticsPerCountriesAndZonesJoiner().run(
        ...
        df_ghg_edunf_by_gas = df_edgar_unfccc_stacked_per_zones_and_countries.groupby(groupby_by_gas).agg(
            ghg=("ghg", "sum"), ghg_unit=("ghg_unit", "first")).reset_index()
        df_ghg_edunf_by_gas.loc[df_ghg_edunf_by_gas[
                                    "ghg"] == 0.0, "ghg"] = np.nan  # TODO - revoir la gestion des données manquantes ghg dans les données UNFCC ANNEXES qui en contient beaucoup. Comment les traiter ? Voir remarque du 29/02.
        ...
        df_ghg_edunf_by_sector = df_edgar_unfccc_stacked_per_zones_and_countries.groupby(groupby_by_sector).agg(
            ghg=("ghg", "sum"), ghg_unit=("ghg_unit", "first")).reset_index()
        df_ghg_edunf_by_sector.loc[df_ghg_edunf_by_sector[
                                       "ghg"] == 0.0, "ghg"] = np.nan  # TODO - revoir la gestion des données manquantes ghg dans les données UNFCC ANNEXES qui en contient beaucoup. Comment les traiter ? Voir remarque du 29/02.
...
class GhgMultiSourcesCombinator:
    ...
    def run(self, df_pik_clean, df_edgar_clean, df_fao_clean, df_cait_sector_stacked, df_cait_gas_stacked,
            df_country):  # TODO - améliorer la jointures des données multi-sources ? données annexe plus utilisées depuis 2017
        """
        ...
        df_edgar_clean["source"] = "EDGAR"
        df_edgar_sum = df_edgar_clean.groupby(["country", "year", "gas", "sector", "ghg_unit", "source"]).agg(
            ghg=("ghg", "sum")).reset_index()
        ...
        df_ghg_multi_by_gas = df_ghg_multi_with_zones.groupby(list_group_by_gas).agg(ghg=("ghg", "sum"), ghg_unit=(
        "ghg_unit", "first")).reset_index()
        df_cait_gas_stacked["source"] = "CAIT"
        df_ghg_multi_by_gas["including_lucf"] = np.nan
        df_cait_stacked_gas = df_cait_gas_stacked[
            (df_cait_gas_stacked["gas"] != "") | (df_cait_gas_stacked["gas"].notnull())]
        ...
        df_ghg_multi_by_sector = df_ghg_multi_with_zones.groupby(list_group_by_sector).agg(ghg=("ghg", "sum"),
                                                                                           ghg_unit=("ghg_unit",
                                                                                                     "first")).reset_index()
